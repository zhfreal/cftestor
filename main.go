...
LOOP:
	for {
		...
		if disableDownload == false {
			...
			if WorkReadyDone == false && len(downloadQueueBuffer) > 0 && (allDownloadTasks-downloadTaskDone) < downloadThread {
				for i := 0; i < downloadThread; i++ {
		...
		if WorkReadyDone == false && (allPingTasks-pingTaskDone) < pingWorkerThread {
			if len(ips) == 0 && ipv6Mode == true {
				ips = GetTestIPs(srcIPS, pingWorkerThread*100)
			}
			if len(ips) > 0 {
